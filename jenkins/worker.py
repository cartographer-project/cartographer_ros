...
CONSTRAINT_STATS_PATTERN = Pattern(
    r'(?s:.*)Score histogram:\nCount:\s+(?P<count>\d+)\s+Min:\s+(?P<score_min>\d+\.\d+)\s+'
    r'Max:\s+(?P<score_max>\d+\.\d+)\s+Mean:\s+(?P<score_mean>\d+\.\d+)')
  ...
  for job_identifier, job_info in stats_dict.iteritems():
    data_string = """[
            ...
            {}
                    ...
                    job_info['max_set_size_kbytes'], job_info['constraints_count'],
                    job_info['constraints_score_min'], job_info['constraints_score_max'],
