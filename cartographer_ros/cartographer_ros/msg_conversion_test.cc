...
TEST(MsgConversion, LaserScanToPointCloud) {
  ...
  EXPECT_TRUE(
      point_cloud[0].position().isApprox(Eigen::Vector4f(1.f, 0.f, 0.f), kEps));
  EXPECT_TRUE(point_cloud[1].position().isApprox(
      Eigen::Vector4f(1.f / std::sqrt(2.f), 1.f / std::sqrt(2.f), 0.f), kEps));
  EXPECT_TRUE(
      point_cloud[2].position().isApprox(Eigen::Vector4f(0.f, 1.f, 0.f), kEps));
  EXPECT_TRUE(point_cloud[3].position().isApprox(
      Eigen::Vector4f(-1.f / std::sqrt(2.f), 1.f / std::sqrt(2.f), 0.f), kEps));
  EXPECT_TRUE(point_cloud[4].position().isApprox(
      Eigen::Vector4f(-1.f, 0.f, 0.f), kEps));
  EXPECT_TRUE(point_cloud[5].position().isApprox(
      Eigen::Vector4f(-1.f / std::sqrt(2.f), -1.f / std::sqrt(2.f), 0.f),
      kEps));
  EXPECT_TRUE(point_cloud[6].position().isApprox(
      Eigen::Vector4f(0.f, -1.f, 0.f), kEps));
