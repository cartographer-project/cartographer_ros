...
PerTrajectorySubmapDisplay::PerTrajectorySubmapDisplay(
    int trajectory_id, ::rviz::Property* submaps_category,
    ::rviz::DisplayContext* display_context, bool visible)
    : id_(trajectory_id), display_context_(display_context) {
...
PerTrajectorySubmapDisplay::~PerTrajectorySubmapDisplay() { submaps_.clear(); }
...
void PerTrajectorySubmapDisplay::ProcessMessage(
  ...
  submaps_.at(id.submap_index)
      ->Update(header, submap_entry, display_context_->getFrameManager());
}
...
void PerTrajectorySubmapDisplay::RemoveUnlistedSubmaps(
  ...
  for (auto it = submaps_.begin(); it != submaps_.end();) {
    if (listed_submaps.count(SubmapId{static_cast<int>(id_), it->first}) == 0) {
      LOG(INFO) << "Removed";
